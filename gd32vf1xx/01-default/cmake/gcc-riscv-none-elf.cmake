...
function(gd32_print_size_of_target TARGET)
...
function(_gd32_generate_file TARGET OUTPUT_EXTENSION OBJCOPY_BFD_OUTPUT)
...
function(gd32_generate_binary_file TARGET)
    _gd32_generate_file(${TARGET} "bin" "binary")
endfunction()

# This function adds post-build generation of the Motorola S-record file from the target ELF.
# The generated file will be placed in the same directory as the ELF file.
function(gd32_generate_srec_file TARGET)
    _gd32_generate_file(${TARGET} "srec" "srec")
endfunction()

# This function adds post-build generation of the Intel hex file from the target ELF.
# The generated file will be placed in the same directory as the ELF file.
function(gd32_generate_hex_file TARGET)
    _gd32_generate_file(${TARGET} "hex" "ihex")
endfunction()

#Generates binary file and copies it with name <TARGET>.elf.bin
function(gd32_generate_elf_bin TARGET)
    gd32_generate_binary_file(${TARGET})
...
function(gd32_generate_elf_hex TARGET)
    gd32_generate_hex_file(${TARGET})
...
function(gd32_generate_lss_file TARGET)
...
if(NOT (TARGET GD32::NoSys))
    add_library(GD32::NoSys INTERFACE IMPORTED)
    target_compile_options(GD32::NoSys INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nosys.specs>)
    target_link_options(GD32::NoSys INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nosys.specs>)
endif()

if(NOT (TARGET GD32::Nano))
    add_library(GD32::Nano INTERFACE IMPORTED)
    target_compile_options(GD32::Nano INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nano.specs>)
    target_link_options(GD32::Nano INTERFACE $<$<C_COMPILER_ID:GNU>:--specs=nano.specs>)
endif()

if(NOT (TARGET GD32::Nano::FloatPrint))
    add_library(GD32::Nano::FloatPrint INTERFACE IMPORTED)
    target_link_options(GD32::Nano::FloatPrint INTERFACE
        $<$<C_COMPILER_ID:GNU>:-Wl,--undefined,_printf_float>
    )
endif()

if(NOT (TARGET GD32::Nano::FloatScan))
    add_library(GD32::Nano::FloatScan INTERFACE IMPORTED)
    target_link_options(GD32::Nano::FloatScan INTERFACE
